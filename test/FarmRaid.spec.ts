...
describe("FarmRaidFacet test", async () => {
  ...
  it("Scout test", async () => {
    ...
    await helpers.bridgeL1toL2(rootDemRebelAddress, demRebelAddress, user, 4);
    for (let id = 0; id < 3; id++) {
      //Only for 3 rebels
      await rebelFarm.connect(user).activateFarm(id, [], []);
    }
  ...
  it("Scout tier test (console)", async () => {
    ...
    const newTokenId = 3;
    rebelFarm.connect(user).activateFarm(newTokenId, [], []);
