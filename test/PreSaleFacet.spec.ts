...
describe.skip('DemRebel PreSaleFacet Test', async () => {
    ...
    it('Check mint limits', async () => {
        ...
        const demRebelPrice = testCfg.demRebelSalePrice;
        const firstLimit = 1000;
        const secondLimit = 5000;
        ...
        {
            const count = 100 + 1;
            const tx = preSaleFacet
                .connect(account)
                .purchaseDemRebels(count, {
...

        {
            ...
            await expect(tx).to.be.revertedWith(
              "SaleFacet: Sale is disabled",
            );
        }
        {
            let tx = await preSaleFacet.connect(accounts[0]).setSaleIsActive(true);
        ...
        {
            let isActive = await preSaleFacet.connect(account).isSaleActive();
    ...
    it('Should buy from whitelisted address', async () => {
        ...
        {
            let tx = await preSaleFacet.setWhitelistActive(true);
            expect((await tx.wait()).status).to.be.equal(1);
        }
        {
            let root = "0x4ab7dfe706af9f8dd8da5e2e026e3ca332f10d64f8d8c2e9dc7e9e61e065d53f";
            let tx = await preSaleFacet.setWhitelistMerkleRoot(root);
        ...
        }
        {
            ...
            const count = 3;
            const tx = await preSaleFacet.connect(account).whitelistSale(proof, count, {
            ...
            assert.equal(tokenIds.length, count + 5000, "error: tokenIds length after buy");
        }
    })
});